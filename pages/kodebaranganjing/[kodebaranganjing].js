import kodebarang from '../../dataset/baranganjing.json'; 
import Layout from '../../layouts/layout';
import Link from 'next/link';

const Baranganjing =(props)=> {
    return(
        <Layout>
        ...
        <Link href="/anjing"><a class="btn btn-primary btn-lg active ml-2 " role="button" aria-pressed="true"><a>KEMBALI</a></a></Link>
        </div>
        </div>
        </div>
        </div>
...
export async function getStaticProps(context){
    let kodebaranganjing = context.params.kodebaranganjing;
    let data = kodebarang.find(x => x.id == kodebaranganjing);

    let {namaproduk, beratproduk, stock, gambar} = data;

    return{
        props : {
            namaproduk, beratproduk, stock, gambar 
        }
    }
}
...
export async function getStaticPaths(){
    return{
        paths : kodebarang.map(data =>(
            {params : {kodebaranganjing : data.id}}
        )),
        fallback: false
    }
}
