import kodbarang from '../../dataset/barangkucing.json'; 
...
const Barang =(props)=> {
    return(
        ...
        <Link href="/kucing"><a class="btn btn-primary btn-lg active ml-2 " role="button" aria-pressed="true"><a>KEMBALI</a></a></Link>
        </div>
        </div>
        </div>
        </div>
...
export async function getStaticProps(context){
    let kodebarangkucing = context.params.kodebarangkucing;
    let data = kodbarang.find(x => x.id == kodebarangkucing);
...
export async function getStaticPaths(){
    return{
        paths : kodbarang.map(data =>(
            {params : {kodebarangkucing : data.id}}
        )),
        fallback: false
    }
}
