import kodbarang from '../../dataset/barangsmallpet.json'; 
...
const Barang =(props)=> {
    return(
        ...
        <Link href="/smallpet"><a class="btn btn-primary btn-lg active ml-2 " role="button" aria-pressed="true"><a>KEMBALI</a></a></Link>
        </div>
        </div>
        </div>
        </div>
...
export async function getStaticProps(context){
    let kodebarangsmallpet = context.params.kodebarangsmallpet;
    let data = kodbarang.find(x => x.id == kodebarangsmallpet);
...
export async function getStaticPaths(){
    return{
        paths : kodbarang.map(data =>(
            {params : {kodebarangsmallpet : data.id}}
        )),
        fallback: false
    }
}
